...
#class Renderer
  ...
  # detail
  # File.open( './conf/frontPopup.yaml' ) { |f|
  #   YAML.load_documents( f ) { |d|
  #     xc = d
  #
  #     # popup - html
	 #  if xc.one_column_flag
		# input = File.read('./template/frontPopupOneColm.html')
	 #  else
		# input = File.read('./template/frontPopupTwoColm.html')
	 #  end
  ...
  #
	 #  # popup - js
	 #  input = File.read('./template/frontPopup.js')
	 #  results = ERB.new(input, nil, '-').result
	 #  output = "./output/js/#{xc.entity_name}EditController.js"
	 #  File.open(output, 'w'){|file| file.write(results)}
  #   }
  # }
